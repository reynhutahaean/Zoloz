...
extern "C"
{
#endif
    //状态回调
    struct ToygerDocCallback
    {
        //算法处理结束 **一次业务只会回调一次**
        //@param cb_receiver:调用方在config时传入的指针,调用方可以通过这个指针绑定到调用config接口的实例
        //@param code:处理结果
        //@param info:采集的证件图片
        void (*handleFinish)(void *cb_receiver, TGResultCode code, const TGDocInfo &info);

        //状态回调 **每一帧调用一次**
        //@param cb_receiver:调用方在config时传入的指针,调用方可以通过这个指针绑定到调用config接口的实例
        //@param state:当前状态，调用方可以根据该参数进行交互提示
        //@param attr:当前属性
        void (*handleUpdate)(void *cb_receiver, const TGDocState &state, const TGDocAttr &attr);
    ...
    bool tg_doc_init(const std::vector<uint8_t> &model, const std::string &license, const std::string &bundleid, void **tg_instance);
    ...
    //@param config[nonnullable]:协议参数
    void tg_doc_config(void *tg_instance, ToygerDocCallback *callback, void *cb_receiver, const TGDocConfig &config);

    //[同步]处理图片帧
    //@param tg_instance[nonnullable]:init时初始化的toyger实例
    //@param frame[nonnullable]:图片数据
    //@return 目前恒为true
    bool tg_doc_process_image(void *tg_instance, TGFrame &frame);

    //[同步]清空算法缓存，比如在重试等场景应用
    //@param tg_instance[nonnullable]:init时初始化的toyger实例
    void tg_doc_reset(void *tg_instance);

    //[同步]清空算法资源，模型、算法实例将被释放，在结束时调用
    //@param tg_instance[nonnullable]:init时初始化的toyger实例
    void tg_doc_release(void *tg_instance);
